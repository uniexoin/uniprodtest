...
export function VendorAnalyticsDashboard() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 w-full bg-background text-foreground min-h-screen p-4 rounded-2xl">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center gap-1 p-2 rounded-2xl text-[11px] font-black uppercase tracking-wider transition-all duration-300 ${isActive
              ...
              >
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-[#8B004A] rounded-2xl shadow-lg shadow-[#8B004A]/25 -z-10"
              ...
              >
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-[#8B004A] rounded-2xl shadow-lg shadow-[#8B004A]/20 -z-10"
                    transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                  />
                )}
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white/40 dark:bg-zinc-900/40 p-8 rounded-[2.5rem] border border-white/60 dark:border-zinc-800 shadow-inner">
        <div>
          <h2 className="text-4xl font-display text-foreground leading-none mb-2">
        ...
        <div className="flex items-center gap-3 bg-white/50 dark:bg-zinc-900/50 p-2 rounded-2xl border border-white/20 dark:border-zinc-800">
          <Badge className="bg-[#8B004A] text-white hover:bg-[#8B004A] border-0 px-4 py-2 rounded-2xl font-caption text-[10px]">
            {overview.bookingsToday || 0} {t('newBookings')}
          </Badge>
          <div className="h-8 w-px bg-slate-200 dark:bg-zinc-800 mx-1" />
          <div className="flex flex-col items-end px-2">
            <span className="text-[10px] font-caption text-slate-400 dark:text-zinc-500 leading-none mb-1">{t('status')}</span>
            <span className="text-xs font-bold text-green-600 dark:text-green-400 leading-none">{t('liveStream')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        {/* Main Chart */}
        <Card className="xl:col-span-2 border-0 shadow-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-3xl overflow-hidden">
          <CardHeader className="border-b border-slate-100 dark:border-zinc-800">
            <div className="flex justify-between items-center">
               ...
               <select className="text-xs font-bold bg-slate-50 dark:bg-zinc-800 border-0 rounded-lg px-2 py-1 outline-none">
        ...
        <Card className="border-0 shadow-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-3xl">
          <CardHeader className="border-b border-slate-100 dark:border-zinc-800">
            <CardTitle className="text-xl font-heading flex items-center gap-2">
              ...
              {(Array.isArray(ledger?.entries) ? ledger.entries : []).slice(0, 5).map((entry: any, i: number) => (
                <div key={entry.id} className="flex items-start gap-4 animate-in slide-in-from-right duration-500" style={{ animationDelay: `${i * 100}ms` }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground truncate">{entry.customerName}</p>
                    <p className="text-xs text-slate-500 dark:text-zinc-400">{new Date(entry.bookingDate).toLocaleDateString()} · {entry.serviceType}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-data text-[#8B004A]">₹{entry.totalAmount}</p>
                    <p className="text-[10px] font-caption text-slate-400">{entry.paymentStatus}</p>
                  </div>
                </div>
              ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="border-0 shadow-2xl bg-white dark:bg-zinc-900 rounded-3xl overflow-hidden">
          <CardHeader className="bg-slate-50/50 dark:bg-zinc-950 border-b border-slate-100 dark:border-zinc-800">
            <CardTitle className="text-lg font-heading">Volume Breakdown</CardTitle>
          </CardHeader>
        ...
        <Card className="border-0 shadow-2xl bg-[#8B004A] text-white rounded-3xl overflow-hidden relative">
          ...
          <CardHeader>
            <CardTitle className="text-white/80 font-caption text-xs">Wallet Status</CardTitle>
          </CardHeader>
          <CardContent className="pt-4 space-y-8">
            <div>
              <p className="text-5xl font-stat text-white">₹{(overview.netEarnings || 0).toLocaleString()}</p>
              <p className="text-white/60 text-sm mt-2 font-medium">Available for immediate withdrawal</p>
            </div>
            <div className="flex gap-4">
              <Button className="bg-white dark:bg-zinc-900/60 text-[#8B004A] hover:bg-white/90 font-bold px-8 rounded-2xl">Withdraw Funds</Button>
            ...
            <div className="relative z-10 p-4 sm:p-6 flex flex-col justify-between h-full">
              <div>
                <div className="flex justify-between items-start mb-2 sm:mb-4">
                  ...
                  {trend && (
                    <div className="flex items-center gap-0.5 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-full text-[9px] sm:text-xs font-black shadow-premium-soft hover-lift">
                ...
                <div className="space-y-0.5 sm:space-y-1">
                  <p className="text-[9px] sm:text-xs font-caption text-slate-400 dark:text-zinc-500 leading-none">{title}</p>
                  <p className="text-xl sm:text-3xl font-stat text-foreground truncate leading-tight group-hover:text-[#8B004A] dark:group-hover:text-rose-400 transition-colors">{value}</p>
                </div>
              </div>
              <div>
                {description && (
                  <p className="text-[8px] sm:text-[10px] text-slate-500 dark:text-zinc-400 font-caption mt-2 sm:mt-4">{description}</p>
                )}
                {subtitle && (
                ...
                {insight && (
                  <div className="mt-3.5 p-2.5 rounded-2xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-200/50 dark:border-white/5 flex items-start gap-2 shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)] group-hover:border-[#8B004A]/20 dark:group-hover:border-rose-500/20 transition-all duration-300">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse mt-0.5 shrink-0 shadow-premium-soft hover-lift shadow-emerald-500/50" />
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <h2 className="font-heading text-2xl text-on-surface">Revenue & Sales</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="glass-panel p-6 rounded-2xl relative overflow-hidden">
            ...
            <p className="font-caption text-sm text-secondary mb-2 uppercase tracking-widest">Total Revenue</p>
            <p className="font-stat text-4xl text-primary mb-1">₹{(overview?.netEarnings || 0).toLocaleString()}</p>
         ...
         <div className="glass-panel p-6 rounded-2xl relative overflow-hidden">
            ...
            <p className="font-caption text-sm text-secondary mb-2 uppercase tracking-widest">Pending Clearances</p>
            <p className="font-stat text-4xl text-on-surface mb-1">₹{(overview?.pendingEarnings || 0).toLocaleString()}</p>
      ...
      {sales && (
        <section className="glass-panel p-8 rounded-3xl mt-8">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-heading text-xl text-on-surface">Revenue Dynamics</h3>
...
export function LedgerSection() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-heading">Ledger Book</h2>
        <div className="flex gap-4">
          ...
          return (
            <Card key={row.id} className={`p-4 border-l-4 transition-all hover:shadow-md ${
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-border/40 text-center">
                <div>
                  <p className="text-[9px] font-caption text-muted-foreground">Amount</p>
                  <p className="text-xs font-data text-foreground">₹{row.totalAmount}</p>
                </div>
                <div>
                  <p className="text-[9px] font-caption text-muted-foreground">Net Earned</p>
                  <p className="text-xs font-data text-green-600">₹{row.netEarned}</p>
                </div>
                <div>
                  <p className="text-[9px] font-caption text-muted-foreground">Due</p>
                  <p className={`text-xs font-data ${row.dueAmount > 0 ? 'text-red-600' : 'text-muted-foreground'}`}>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-900 text-white uppercase text-xs">
              <tr>
                ...
                <th className="px-6 py-5 text-right">Due</th>
            ...
            <tbody className="divide-y divide-border bg-card">
              {(Array.isArray(ledger?.entries) ? ledger.entries : []).map((row: any) => {
                ...
                `}>
                  <td className="px-6 py-5 whitespace-nowrap text-xs font-data">{formattedDate}</td>
                  <td className="px-6 py-5 font-medium text-gray-900">{row.customerName}</td>
                  <td className="px-6 py-5 text-gray-500">{row.serviceName}</td>
                  <td className="px-6 py-5 text-right font-data">₹{row.totalAmount}</td>
                  <td className="px-6 py-5 text-right text-green-600 font-data">₹{row.netEarned}</td>
                  <td className="px-6 py-5 text-right text-red-600 font-data">{row.dueAmount > 0 ? `₹${row.dueAmount}` : '-'}</td>
            ...
            <tfoot className="bg-slate-50 dark:bg-zinc-800 border-t-2 border-slate-200 font-bold">
              <tr>
                <td colSpan={3} className="px-6 py-5 text-right">PAGE TOTALS:</td>
                <td className="px-6 py-5 text-right">₹{(ledger?.totals?.totalRevenue || 0).toLocaleString()}</td>
                <td className="px-6 py-5 text-right text-green-600">₹{(ledger?.totals?.totalNetEarned || 0).toLocaleString()}</td>
                <td className="px-6 py-5 text-right text-red-600">₹{(ledger?.totals?.totalDue || 0).toLocaleString()}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white/40 dark:bg-zinc-900/40 p-6 rounded-[2rem] border border-white/60 dark:border-zinc-800 shadow-inner">
        <div>
          <h2 className="text-3xl font-heading flex items-center gap-3">
        ...
        {/* Daily Revenue Bar */}
        <div className="flex flex-col bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-premium-soft hover-lift border border-slate-100 dark:border-zinc-800 text-center sm:text-right">
            <span className="text-xs font-caption text-slate-500 dark:text-zinc-400">Today's Total Revenue</span>
            <span className="text-3xl font-stat text-[#8B004A] dark:text-rose-400 leading-none mt-1">₹{todayRevenue.toLocaleString()}</span>
        </div>
      </div>
          ...
          <Card key={v._id} className={`overflow-hidden border-t-4 transition-all hover:shadow-lg bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md flex flex-col ${
            ...
            <div className="relative h-48 w-full bg-slate-100 dark:bg-zinc-800">
               ...
               <div className="absolute bottom-3 left-3 right-3 flex justify-between items-end">
                  <div className="text-white">
                    <h3 className="font-bold text-lg leading-tight shadow-premium-soft hover-lift">{v.name}</h3>
            ...
            <CardContent className="p-5 flex-1 flex flex-col">
                {v.currentStatus === 'dispatched' && v.currentBooking ? (
                  <div className="bg-slate-50 dark:bg-zinc-800/50 p-4 rounded-2xl mb-4 text-sm space-y-3 flex-1 border border-slate-100 dark:border-white/5">
                    ...
                    <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-2">
                      ...
                      <span className="font-data text-green-600 dark:text-green-400">₹{v.currentBooking.totalAmount || v.pricePerDay}</span>
                    </div>
                  ...
                  <div className="mb-4 flex-1 flex flex-col items-center justify-center p-6 border-2 border-dashed border-purple-200 dark:border-purple-900 rounded-2xl bg-purple-50/50 dark:bg-purple-900/10">
                  ...
                  <div className="mb-4 flex-1 flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-200 dark:border-zinc-700 rounded-2xl bg-slate-50/50 dark:bg-zinc-950 dark:bg-zinc-800/30">
                     <p className="text-sm text-muted-foreground font-bold mb-1">Available for Booking</p>
                     <p className="text-2xl font-stat text-foreground">₹{v.pricePerDay}<span className="text-xs text-muted-foreground font-body"> / day</span></p>
                  </div>
                )}
...
function VehicleOperationsHistory() {
  ...
  return (
    <div className="mt-12 space-y-4">
      <h3 className="text-xl font-heading">Operations Ledger</h3>
      ...
      {/* Desktop Table View */}
      <Card className="border-0 shadow-lg overflow-hidden bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl hidden md:block">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-zinc-800 border-b border-slate-100 dark:border-zinc-800">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100">
              {operations?.map((op: any) => (
                <tr key={op.id} className="hover:bg-slate-50 dark:bg-zinc-800 transition-colors">
...
export function LaundrySection() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-heading">Laundry Kanban Pipeline</h2>
        {laundry && (
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black uppercase whitespace-nowrap transition-all ${
                isActive
                  ...
                  : 'bg-white/50 text-slate-500 dark:text-zinc-400 border border-slate-200 hover:bg-white dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700'
              }`}
            >
              <span>{stage.label}</span>
              <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${
                isActive ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:bg-zinc-700 dark:text-zinc-300'
              }`}>{count}</span>
            </button>
          );
        })}
            ...
            return (
              <div key={stage.id} className="flex flex-col w-full">
                <div className={`p-3.5 rounded-t-xl border-t border-l border-r font-bold flex justify-between items-center ${stage.color}`}>
                  ...
                  <span className="bg-white/80 dark:bg-zinc-800/80 px-2.5 py-0.5 rounded-full text-xs font-black shadow-premium-soft hover-lift">{stageOrders.length}</span>
                </div>
                <div className="bg-white/30 dark:bg-zinc-900/30 backdrop-blur-sm border p-4 flex-1 rounded-b-xl space-y-3 min-h-[300px]">
                  {stageOrders.map((order: any) => (
                    <Card key={order._id} className="shadow-premium-soft hover-lift border border-slate-100 dark:border-zinc-800/50 hover:border-[#8B004A]/30 transition-all duration-300">
                      <CardContent className="p-4">
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-data text-[#8B004A]">₹{order.totalAmount}</span>
                          {/* Quick Action Button */}
                          <div className="w-1/2">
                  ...
                  {stageOrders.length === 0 && (
                    <div className="text-center py-12 text-xs text-muted-foreground font-semibold border-2 border-dashed border-slate-200 dark:border-zinc-800/80 rounded-2xl">
            ...
            return (
              <div key={stage.id} className="w-[300px] shrink-0 snap-center flex flex-col h-[calc(100vh-220px)]">
                ...
                <div className="bg-slate-50 dark:bg-zinc-900 border p-3 flex-1 overflow-y-auto rounded-b-xl space-y-3">
                  {stageOrders.map((order: any) => (
                    <Card key={order._id} className="shadow-premium-soft hover-lift cursor-grab hover:border-[#8B004A]/30 transition-colors">
